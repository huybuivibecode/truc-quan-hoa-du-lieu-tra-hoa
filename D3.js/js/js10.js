d3.csv("data/data.csv").then(function (raw) {
  ...
  // Đếm số đơn hàng uniq theo nhóm + tháng
  ...
  // Đếm uniq đơn hàng theo mặt hàng trong nhóm + tháng
  ...
  groups.forEach(([nhomTen, itemArr], idx) => {
    ...
    const series = itemArr.map(([mhKey, recs]) => {
      ...
      const pts = d3.range(1, 13).map(m => ({ Thang: m, TyLe: byMonth.get(m) ?? 0 }));
    ...
    let yDomain;
    if (nhomMa === "BOT") yDomain = [0, 2.0];
    else if (nhomMa === "SET") yDomain = [0.05, 0.25];
    else if (nhomMa === "THO") yDomain = [0.1, 0.35];
    else if (nhomMa === "TTC") yDomain = [0.25, 0.8];
    else if (nhomMa === "TMX") yDomain = [0.25, 0.55];
    else {
